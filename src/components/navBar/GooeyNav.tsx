...
export interface GooeyNavProps {
  ...
  /** Enable Next.js Link prefetching for faster navigation (default: true) */
  prefetch?: boolean;
  /** Control scroll behavior on navigation (default: true) */
  scroll?: boolean;
}
...
const GooeyNav: React.FC<GooeyNavProps> = ({
  ...
  // Memoize active index calculation based on current pathname
  ...
  // Memoize noise function
  const noise = useCallback((n = 1) => n / 2 - Math.random() * n, []);
  
  // Memoize getXY calculation
  ...
  const createParticle = useCallback(
    (i: number, t: number, d: [number, number], r: number) => {
      ...
      const randomChartColor = chartColors[Math.floor(Math.random() * chartColors.length)];
      return {
        start: getXY(d[0], particleCount - i, particleCount),
        end: getXY(d[1] + noise(7), particleCount - i, particleCount),
        time: t,
        scale: 1 + noise(0.2),
        chartColor: randomChartColor,
        rotate: rotate > 0 ? (rotate + r / 20) * 10 : (rotate - r / 20) * 10,
      };
    },
    [noise, getXY, particleCount]
  );
  // Memoize makeParticles function
    ...
    (element: HTMLElement) => {
      ...
      for (let i = 0; i < particleCount; i++) {
        ...
        setTimeout(() => {
          ...
          particle.style.setProperty("--color", `var(--chart-${p.chartColor})`);
          ...
          setTimeout(() => {
            try {
              ...
              if (element.contains(particle)) {
                ...
                console.error("Failed to remove particle: not a child of element", {
                  element,
                  particle,
                });
              }
  ...
  // Memoize updateEffectPosition function
  ...
  const handleClick = useCallback(
    ...
    [activeIndex, updateEffectPosition, makeParticles, startTransition]
  );
  
  // Memoize handleKeyDown with useCallback
  ...
  useEffect(() => {
    const index = items.findIndex(item => item.href === pathname);
    const newIndex = index !== -1 ? index : initialActiveIndex;
    if (newIndex !== activeIndex) {
      setActiveIndex(newIndex);
    }
  }, [pathname, items, initialActiveIndex, activeIndex]);
  ...
  return (
    <>
      ...
      <div
        className="relative bg-muted rounded-full shadow-lg"
        ref={containerRef}
        role="navigation"
              ...
              return (
                <li
                  ...
                  className={`rounded-full px-2 py-0.5 relative cursor-pointer transition-[background-color_color_box-shadow] duration-300 ease shadow-[0_0_0.5px_1.5px_transparent] text-foreground ${
                    isActive ? "active" : "hover:animate-pulse"
                  }`}>
                  <Link
                    href={item.href}
                    prefetch={prefetch}
                    ...
                    className={`rounded-full outline-none py-[0.6em] px-[1em] inline-block focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 transition-all ${
